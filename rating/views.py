...
from .models import Profession, Question, Comment
...
def profession(request, prof_slug):
    ...
    return render(request, 'profession.html', {'prof_data': prof_data, 'questions': questions})


def question(request, question_id):
    question_data = Question.objects.get(id=question_id)
    comments = Comment.objects.filter(question__id=question_data.id)
    return render(request, 'question.html', {'question_data': question_data, 'comments': comments})
